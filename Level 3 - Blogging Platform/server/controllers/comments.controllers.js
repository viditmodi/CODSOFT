...
const createNewComment = async (req, res) => {
  try {
    ...
    const updatedAccount = await AccountCollection.findOneAndUpdate(
      ...
      { total_comments: accountData.total_comments + 1 },
      { new: true }
    );
    ...
    if (comment_for === "post") {
      ...
      const updatedPost = await PostsCollection.findOneAndUpdate(
        { postID: id },
        { total_comments: postData.total_comments + 1 }
      );
    }
    const blogData = await BlogsCollection.findOne({
      blogID: updatedPost.blogID,
    });
    const updatedblog = await BlogsCollection.findOneAndUpdate(
      { blogID: updatedPost.blogID },
      { total_comments: blogData.total_comments + 1 }
    );
    res
      .status(200)
      .send({
...
const getAllComments = async (req, res) => {
  try {
    ...
    res
      .status(200)
      .send({ status: true, message: "comment found", data: commentsArray });
  } catch (error) {
